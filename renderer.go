...
func (r *Renderer) WithCtx(ctx interface{}) {
	if ctx == nil {
...
func (r *Renderer) Line(line string, a ...interface{}) {
	r.buf.WriteString(formatLine(line, r.defaultCtx, a...))
	r.buf.WriteByte('\n')
}

// Rawl renders a line without format
func (r *Renderer) Rawl(line string) {
	r.buf.WriteString(line)
	r.buf.WriteByte('\n')
}

// Newl renders empty line
func (r *Renderer) Newl() {
	r.buf.WriteByte('\n')
}

// RenderAutogen format collected code with header comment about being autogenerated and advice
// not to touch it manually
func (r *Renderer) RenderAutogen(appName, pkg string, imports []ImportsGroup) (io.Reader, error) {
	return r.render(fmt.Sprintf("Code generate with %s. DO NOT EDIT.", appName), pkg, imports)
}

// Render format collected code with given package comment
func (r *Renderer) Render(pkgComment, pkg string, imports []ImportsGroup) (io.Reader, error) {
	return r.render(pkgComment, pkg, imports)
}

func (r *Renderer) render(pkgComment string, pkg string, imports []ImportsGroup) (io.Reader, error) {
	var buf bytes.Buffer
	pkgComment = strings.TrimRight(pkgComment, "\n")
	switch strings.Count(pkgComment, "\n") {
	case 0:
		if pkgComment != "" {
			buf.WriteString("// ")
